...
const FormSelect: React.FC<FormSelectProps> = ({
    ...
    return (
            <div className='min-w-[159px] h-10 relative z-[1]'>
                {/* btn select */}
                <div
                    className='font-montserrat absolute top-0 z-[1] left-0 right-0 text-[15px] px-3 h-9 rounded-[20px] bg-white font-medium flex items-center border border-[#cdcdcd]  cursor-pointer select-none'
                    onClick={() => onClick(optionType)}
                    >
                    <span className='flex-[100%]'>{title}</span>
                    <IoChevronDown size={25} className='ml-auto' />
                </div>
                <div className={`absolute top-0 left-0 right-0 flex flex-col min-w-[159px] bg-white rounded-[20px] shadow-md border pt-10 pb-2 px-2 
